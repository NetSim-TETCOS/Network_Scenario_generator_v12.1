...
namespace NetSim_Scenario_Generation_IOT
{
    class AddNetworkElement
    {
        ...
        */
        ...
        /*
          func: function for adding an input string to string "0000" 
          params: s1- input string 
          return: return resultant string after addition
        */
        public string str_addition(string s1)
        {
            string sum="0000";
            StringBuilder str = new StringBuilder(sum);
            int i,j=3, len = s1.Length;
            for(i=len-1;i>=0;i--,j--)
            {
                int a = str[j] - '0';
                int b = s1[i] - '0';
        ...
        /*
          func: generates an ipv6
          params: id- rank(numbered) of device for which ipv6 has to be assigned 
          return: return the generated ipv6
        */
        public string next_ipv6(int id)
        {
            string first_6_segment = "fdec:3017:e256:9bb8:1fe7:6bb4";
            int _7th_segment = 0, _8th_segment = 0;

            _8th_segment += id % 10000;
            _7th_segment += (id / 10000) % 10000;
            string ip = first_6_segment + ":" + str_addition(Convert.ToString(_7th_segment)) + ":" + str_addition(Convert.ToString(_8th_segment));
            return ip;
        }
        ...
        */
        ...
        /*
          func: adds a wireless_sensor(device) as child node to DEVICE_CONFIGUARTION(parent node)
          params: deviceConfig:-reference to parent node(DEVICE_CONFIGUARTION)
                  device_attribute:- attributs of wireless_sensor
                  pos_3d_attribute:-postion info of wireless_sensor
                  interface_variables:- interface info of wireless_sensor
                  other_info- extra info about the wireless_sensor
          return: doesnt return
        */
        public void add_wireless_sensor(XmlNode deviceConfig, DEVICE device_attribute, POS_3D pos_3d_attribute, INTERFACE[] interface_variables,OTHER_INFO other_info, string config_helper_location)
        {
            XmlNode device = nsWriter.add_element_from_file_with_format(deviceConfig, config_helper_location + "\\ConfigHelper\\Wireless_Sensor\\Device.txt",
                  Convert.ToString(device_attribute.DEVICE_ID),
                  Convert.ToString(device_attribute.DEVICE_NAME),
                  Convert.ToString(device_attribute.WIRESHARK_OPTION)
                );
            ...
            nsWriter.add_element_from_file_with_format(device, config_helper_location + "\\ConfigHelper\\Wireless_Sensor\\Mobility_Model\\" + mobility_file_name,
                  Convert.ToString(pos_3d_attribute.X_OR_LON),
                  Convert.ToString(pos_3d_attribute.Y_OR_LAT)
                );
            ...
            for (int i = 0; i < device_attribute.INTERFACE_COUNT; i++)
            {
                XmlNode _interface = nsWriter.add_element_from_file_with_format(device, config_helper_location + "\\ConfigHelper\\Wireless_Sensor\\Interface.txt",
                    Convert.ToString(interface_variables[i].DEFAULT_GATEWAY),
                    Convert.ToString(interface_variables[i].IP_ADDRESS),
                    Convert.ToString(interface_variables[i].MAC_ADDRESS));
                ...
                nsWriter.add_element_from_file(_interface, config_helper_location + "\\ConfigHelper\\Wireless_Sensor\\" + power_filename);
            }


            nsWriter.add_element_from_file(device, config_helper_location + "\\ConfigHelper\\Wireless_Sensor\\Application_Layer.txt");
            nsWriter.add_element_from_file(device, config_helper_location + "\\ConfigHelper\\Wireless_Sensor\\Transport_Layer.txt");
            XmlNode routing_protocol = nsWriter.add_element_from_file(device, config_helper_location + "\\ConfigHelper\\Wireless_Sensor\\Network_Layer.txt");
        }

        /*
          func: adds a gateway(device) as child node to DEVICE_CONFIGUARTION(parent node)
          params: deviceConfig:-reference to parent node(DEVICE_CONFIGUARTION)
                  device_attribute:- attributs of gateway
                  pos_3d_attribute:-postion info of gateway
                  interface_variables:- interface info of gateway
          return: doesnt return
        */
        public void add_gateway(XmlNode deviceConfig, DEVICE device_attribute, POS_3D pos_3d_attribute, INTERFACE[] interface_variables, string config_helper_location)
        {
            XmlNode device = nsWriter.add_element_from_file_with_format(deviceConfig, config_helper_location + "\\ConfigHelper\\Gateway\\Device.txt",
                  Convert.ToString(device_attribute.DEVICE_ID),
                  Convert.ToString(device_attribute.DEVICE_NAME),
                  Convert.ToString(device_attribute.INTERFACE_COUNT),
                  Convert.ToString(pos_3d_attribute.X_OR_LON),
            ...
            for (int i = 0; i < device_attribute.INTERFACE_COUNT; i++)
            {
                if (interface_variables[i].INTERFACE_TYPE.Equals("WAN"))
                {
                    nsWriter.add_element_from_file_with_format(device, config_helper_location + "\\ConfigHelper\\Gateway\\Wan\\Interface.txt",
                         Convert.ToString(interface_variables[i].ID),
                         Convert.ToString(interface_variables[i].DEFAULT_GATEWAY),
                         Convert.ToString(interface_variables[i].IP_ADDRESS),
                         Convert.ToString(interface_variables[i].SUBNET_MASK),
                ...
                else if(interface_variables[i].INTERFACE_TYPE.Equals("ZIGBEE"))
                {
                    nsWriter.add_element_from_file_with_format(device, config_helper_location + "\\ConfigHelper\\Gateway\\Zigbee\\Interface.txt",
                         Convert.ToString(interface_variables[i].ID),
                         Convert.ToString(interface_variables[i].DEFAULT_GATEWAY),
                         Convert.ToString(interface_variables[i].IP_ADDRESS),
                         Convert.ToString(interface_variables[i].MAC_ADDRESS));
            ...
            XmlNode routing_protocol = nsWriter.add_element_from_file(device, config_helper_location + "\\ConfigHelper\\Gateway\\Application_Layer.txt");
            XmlNode protocol_property = nsWriter.add_element_from_file(routing_protocol, config_helper_location + "\\ConfigHelper\\Gateway\\Application_Layer_Routing_Protocol.txt");
            XmlNode property_interface = nsWriter.add_element_from_file(protocol_property, config_helper_location + "\\ConfigHelper\\Gateway\\Application_Layer_Routing_Protocol_Property.txt");
            for (int i = 0; i < device_attribute.INTERFACE_COUNT; i++)
            {
                if (interface_variables[i].INTERFACE_TYPE.Equals("WAN"))
                    nsWriter.add_element_from_file_with_format(property_interface, config_helper_location + "\\ConfigHelper\\Gateway\\Application_Layer_Routing_Protocol_Property_Interface_Wan.txt",
                    Convert.ToString(interface_variables[i].ID));
            }
            nsWriter.add_element_from_file(device, config_helper_location + "\\ConfigHelper\\Gateway\\Transport_Layer.txt");
            nsWriter.add_element_from_file(device, config_helper_location + "\\ConfigHelper\\Gateway\\Network_Layer.txt");
        }
        ...
        */
        public void add_deviceConfig(XmlNode parent, int device_count, DEVICE_CONTAINER[] device_container,OTHER_INFO other_info, string config_helper_location)
        {
            ...
            for (int i = 0; i < deviceCount; i++)
            {
                ...
                else if(device_container[i].device.DEVICE_TYPE.Equals("IOT_Sensors"))
                    add_wireless_sensor(deviceConfig, device_container[i].device, device_container[i].pos_3d, device_container[i]._interface,other_info, config_helper_location);
                else if(device_container[i].device.DEVICE_TYPE.Equals("LOWPAN_Gateway"))
                    add_gateway(deviceConfig, device_container[i].device, device_container[i].pos_3d, device_container[i]._interface, config_helper_location);
            }
        ...
        */
        public void add_connection(XmlNode parent, int link_count, LINK[] link, string config_helper_location)
        {
            ...
            for (int i = 0; i < link_count; i++)
            {
                ...
                if (link[i].link_type.Equals("M_to_M"))
                {
                    link_element = nsWriter.add_element_from_file_with_format(con, config_helper_location + "\\ConfigHelper\\Adhoc_Link\\Link.txt",
                    ...
                    folder_name = "Adhoc_Link";
                }
                else
                {
                    link_element = nsWriter.add_element_from_file_with_format(con, config_helper_location + "\\ConfigHelper\\Wired_Link\\Link.txt",
                     Convert.ToString(link[i].DEVICE_COUNT),
                     Convert.ToString(link[i].LINK_ID),
                     Convert.ToString(link[i].LINK_NAME));
                    folder_name = "Wired_Link";

                }
                for (int j = 0; j < link[i].DEVICE_COUNT; j++)
                {
                    nsWriter.add_element_from_file_with_format(link_element, config_helper_location + "\\ConfigHelper\\" +folder_name+"\\Link_Device.txt",
                ...
                if (link[i].link_type.Equals("M_to_M"))
                {
                    nsWriter.add_element_from_file_with_format(link_element, config_helper_location + "\\ConfigHelper\\Adhoc_Link\\Link_Medium_Property.txt");
                }
                else if (link[i].link_type.Equals("P_to_P_Router_WiredNode"))
                {
                    nsWriter.add_element_from_file_with_format(link_element, config_helper_location + "\\ConfigHelper\\Wired_Link\\Link_Medium_Property.txt");
                }
                else
                {
                    nsWriter.add_element_from_file_with_format(link_element, config_helper_location + "\\ConfigHelper\\Wired_Link\\Link_Medium_Property.txt");
                    nsWriter.add_element_from_file_with_format(link_element, config_helper_location + "\\ConfigHelper\\Wired_Link\\Link_Failure.txt");
                }
            }
        }
